...
def train_model(niters, training_trajs, dt, sample_length, batch_size, learning_rate, scheduling_factor, scheduling_freq, nn_depth, nn_width, loss_func):    
    ...
    for itr in range(1, niters + 1):
        ...
        if loss_func == 'all':
            ...
            loss = torch.mean(torch.abs(pred_y[:,:,:dim] - batch_y[:,:,:dim]))
        elif loss_func == 'pos':
            loss = torch.mean(torch.abs(pred_y[:,:,dim:] - batch_y[:,:,dim:]))
        loss.backward() 
        optimizer.step()
